...
- The extensionless Lua `modem` RPC file preserves the confirmed stock method names. A community handler can answer only for its configured USB IDs. Result handlers may fill modem-specific fields after successful stock calls; supplemental stock calls are completed before the protected, non-yielding handler runs, and handler errors preserve the unmodified stock result. All other calls use the stock `.so` path.
...
- `rpc-drivers/vos5g.lua` preserves the stock QMI `get_network_info` result and fills only a missing `cell_info.mode`, using the stock `get_sim_status` technology value for USB ID `05c6:9064`. A VOS-only watcher reads Qualcomm NAS `Get RF Band Information` and DSD `Get System Status` through short-lived, independently allocated clients, then atomically caches the active band and an explicitly reported `SA`/`NSA` mode without blocking the HTTP worker. The compact display uses forms such as `SA n71`, `NSA n78+B2`, and `LTE B2`. DSD service-option bits, never RF band membership, determine SA versus NSA. Missing or stale DSD data falls back to `NR nXX`; missing or stale band data falls back to the technology-only `NR` or `LTE` label.
...
- `vos5g-qmi` preparation and the display watcher serialize on the same per-bus lock, so the package's own QMI users never interleave on the control device. Preparation blocks on contention and reports the winning caller's real outcome; the watcher skips a poll while preparation holds the lock. A completed preparation is stamped against the current USB connection (`devnum`), so duplicate bind events short-circuit while a physical detach/attach forces a full re-preparation.
